#******************************************************************************
# 0. Identification -------------------------------------------------------
# Title: Multivariate analysis code for a research paper on
...
results_mm_z_sin_stgo <- list(
  elsoc_2016 = map(
    varsdep,
    ~ estimate_mm(.x, datos = elsocs_sin_stgo[[1]]),
    .progress = TRUE
  ) %>%
    set_names(varsdep),
  elsoc_2019 = map(
    varsdep,
    ~ estimate_mm(.x, datos = elsocs_sin_stgo[[2]]),
    .progress = TRUE
  ) %>%
    set_names(varsdep),
  elsoc_2022 = map(
    varsdep,
    ~ estimate_mm(.x, datos = elsocs_sin_stgo[[3]]),
    .progress = TRUE
  ) %>%
    set_names(varsdep)
)

rm(list = ls()[!ls() %in% c("results_mm_z_sin_stgo", "date")])

# 5. Save ---------------------------------------------------------------------
save.image(glue("output/models/main/results_mm_z_sin_stgo.RData"))
